/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AuthorizationUpdateManyWithoutCustomersInput } from "./AuthorizationUpdateManyWithoutCustomersInput";
import { ValidateNested, IsOptional, IsDate, IsString } from "class-validator";
import { Type } from "class-transformer";
import { CreditCheckUpdateManyWithoutCustomersInput } from "./CreditCheckUpdateManyWithoutCustomersInput";

@InputType()
class CustomerUpdateInput {
  @ApiProperty({
    required: false,
    type: () => AuthorizationUpdateManyWithoutCustomersInput,
  })
  @ValidateNested()
  @Type(() => AuthorizationUpdateManyWithoutCustomersInput)
  @IsOptional()
  @Field(() => AuthorizationUpdateManyWithoutCustomersInput, {
    nullable: true,
  })
  authorizations?: AuthorizationUpdateManyWithoutCustomersInput;

  @ApiProperty({
    required: false,
    type: () => CreditCheckUpdateManyWithoutCustomersInput,
  })
  @ValidateNested()
  @Type(() => CreditCheckUpdateManyWithoutCustomersInput)
  @IsOptional()
  @Field(() => CreditCheckUpdateManyWithoutCustomersInput, {
    nullable: true,
  })
  creditChecks?: CreditCheckUpdateManyWithoutCustomersInput;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  dateOfBirth?: Date | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  firstName?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  lastName?: string | null;
}

export { CustomerUpdateInput as CustomerUpdateInput };
